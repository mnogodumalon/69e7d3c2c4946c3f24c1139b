...
import { Fermenter2LivebildViewDialog } from '@/components/dialogs/Fermenter2LivebildViewDialog';
...
const FERMENTER2LIVEBILD_FIELDS = [
];

const ENTITY_TABS = [
  { key: 'fermenter_2_livebild', label: 'Fermenter 2 Livebild', pascal: 'Fermenter2Livebild' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('fermenter_2_livebild');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'fermenter_2_livebild': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'fermenter_2_livebild': return (data as any).fermenter2Livebild as Fermenter2Livebild[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'fermenter_2_livebild': return FERMENTER2LIVEBILD_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'fermenter_2_livebild': return {
        create: (fields: any) => LivingAppsService.createFermenter2LivebildEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateFermenter2LivebildEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteFermenter2LivebildEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'fermenter_2_livebild' || dialogState?.entity === 'fermenter_2_livebild') && (
        <Fermenter2LivebildDialog
          open={createEntity === 'fermenter_2_livebild' || dialogState?.entity === 'fermenter_2_livebild'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'fermenter_2_livebild' ? handleUpdate : (fields: any) => handleCreate('fermenter_2_livebild', fields)}
          defaultValues={dialogState?.entity === 'fermenter_2_livebild' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Fermenter2Livebild']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Fermenter2Livebild']}
        />
      )}
      {viewState?.entity === 'fermenter_2_livebild' && (
        <Fermenter2LivebildViewDialog
          open={viewState?.entity === 'fermenter_2_livebild'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'fermenter_2_livebild', record: r }); }}
        />
      )}
