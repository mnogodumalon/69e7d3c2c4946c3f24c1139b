...
import type { Fermenter2Livebild, CreateFermenter2Livebild } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getFermenter2Livebild(): Promise<Fermenter2Livebild[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.FERMENTER_2_LIVEBILD}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Fermenter2Livebild[];
    return enrichLookupFields(records, 'fermenter_2_livebild');
  }
  static async getFermenter2LivebildEntry(id: string): Promise<Fermenter2Livebild | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.FERMENTER_2_LIVEBILD}/records/${id}`);
    const record = { record_id: data.id, ...data } as Fermenter2Livebild;
    return enrichLookupFields([record], 'fermenter_2_livebild')[0];
  }
  static async createFermenter2LivebildEntry(fields: CreateFermenter2Livebild) {
    return callApi('POST', `/apps/${APP_IDS.FERMENTER_2_LIVEBILD}/records`, { fields: cleanFieldsForApi(fields as any, 'fermenter_2_livebild') });
  }
  static async updateFermenter2LivebildEntry(id: string, fields: Partial<CreateFermenter2Livebild>) {
    return callApi('PATCH', `/apps/${APP_IDS.FERMENTER_2_LIVEBILD}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'fermenter_2_livebild') });
  }
  static async deleteFermenter2LivebildEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.FERMENTER_2_LIVEBILD}/records/${id}`);
  }
